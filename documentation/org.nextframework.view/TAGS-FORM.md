...
When `ajaxUpload="true"`:

- the form stays as a regular `application/x-www-form-urlencoded` submit when all file inputs are AJAX-enabled
- each selected file is uploaded first to `UploadFileServlet` (`/uploadfile`)
- the file input is disabled before the final form submit to avoid multipart mode
- the hidden field `<propertyName>_tempFileToken` stores the temporary file token
- if validation fails and the form is rendered again, the same file can be rebound from `_tempFileToken`
- if the user selects a different file, the new upload overwrites the previous token

### File Input Notes

- `showDeleteButton="true"` clears `_fileObject` and `_tempFileToken`
- persisted files (`cdfile > 0`) continue to use `_fileObject`
- non-persisted files use `_tempFileToken`, regardless of whether they came from AJAX upload or from a validation roundtrip
- use the JavaScript submit function generated by `n:form`; do not call `form.submit()` directly, or the AJAX upload pre-processing will be skipped
...
Use the JavaScript submit function generated by `n:form` instead of calling `form.submit()` directly.
Direct `form.submit()` bypasses the form tag processing flow, including multipart adjustments and pre-submit hooks such as AJAX file upload handling.
...
1. If you add or rename a tag attribute in Java, review the corresponding TLD files in `resources/META-INF/*.tld`.
2. Before adding an attribute to a TLD, confirm the exact Java tag class referenced by that tag. Tags with the same name in different TLDs may point to different classes.
3. If you change shared JavaScript APIs, review JSPs and Java classes that emit inline JavaScript calls.
