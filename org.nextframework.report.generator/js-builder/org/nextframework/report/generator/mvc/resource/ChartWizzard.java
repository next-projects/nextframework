...
public class ChartWizzard {
	...
	ChartConfiguration chartConfigurationToSave;
	...
	private void configureGroupItens() {
		...
		next.effects.hideProperty(chartGroupByLevel);
		for (String key : designer.fields) {
			Map<String, Object> properties = designer.fields.$get(key);
	...
	private void configureGroupDateLevel() {
		...
		if (isdate != null && isdate.equals("true")) {
			next.effects.showProperty(chartGroupByLevel);
		} else {
			next.effects.hideProperty(chartGroupByLevel);
		}
	}
	...
	private void gotoConfirmationView(Element button) {
		...
		chartConfigurationToSave = new ChartConfiguration(getSelectedChartType(),
		...
		for (int i = 0; i < options.length; i++) {
			...
			String aggregateFunction = (String) properties.$get(AGGREGATE_FUNCTION);
			String label = (String) properties.$get(SERIE_LABEL);
			chartConfigurationToSave.addSerie(option.value, aggregateFunction, label);
		}

		setSpanText("chartTitleSpan", chartConfigurationToSave.title);
		setSpanText("chartGroupSpan", chartConfigurationToSave.groupProperty);
		setSpanText("chartValueSpan", chartConfigurationToSave.valueProperty.equals("count") ? "Contagem dos itens" : chartConfigurationToSave.valueProperty + " (" + chartConfigurationToSave.valueAggregate + ")");
	...
	protected void saveConfiguration() {
		if (editing == null) {
			designer.addChart(chartConfigurationToSave);
		} else {
			designer.updateChart(editing.option, chartConfigurationToSave);
		}
	}
	...
	protected boolean next(Element button) {
		if (currentPage == 110) {
			gotoChartConfig();
		} else if (currentPage == 121) {
			gotoConfirmationView(button);
		} else if (currentPage == 131) {
			saveConfiguration();
			return true;
		}
		return false;
