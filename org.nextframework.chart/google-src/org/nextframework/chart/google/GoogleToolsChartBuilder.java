...
public class GoogleToolsChartBuilder extends JavascriptBuilder implements GoogleVisualization, Html {
	...
	private DataTable transformChartDataToDataTable(){
		...
		if(ChartUtils.hasText(this.chart.getStyle().getValuePattern())){
			NumberFormat formatter = new NumberFormat(this.chart.getStyle().getValuePattern());
	...
	private String getFormatedValue(int j, Number value) {
		PropertyEditor formatter = this.chart.getStyle().getValuesFormatterForSerie(j);
		if(formatter != null){
	...
	private void createAndDrawChart(final String id, DataTable data) {
		...
		Map vAxis = map();
		if(ChartUtils.hasText(this.chart.getData().getSeriesTitle())){
