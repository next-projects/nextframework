...
public class ChartDataRegrouper {
	...
	public void regroup(ChartData data){
		if(maximumTopBySerie >= data.series.length + 1){
			return;
		}
		SortedSet<Integer> topSeries = new TreeSet<Integer>();
		List<ChartRow> rows = data.data;
		...
		data.series = keepPositions(data.series, new Comparable<?>[topSeries.size()+1], topSeries, new Grouper<Comparable<?>>(){
			@Override
